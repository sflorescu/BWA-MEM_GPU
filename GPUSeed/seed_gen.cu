...
extern "C" int seed(int argc, char **argv, int u) {
	...
	strcpy(str, prefix); strcat(str, ".bwt");  bwt = bwt_restore_bwt_gpu(str);
	free(str);
	str = (char*)calloc(strlen(prefix) + 10, 1);
	strcpy(str, prefix); strcat(str, ".sa");  bwt_restore_sa_gpu(str, bwt);
	free(str);
	uint32_t i;
	int count_0 = 0;
